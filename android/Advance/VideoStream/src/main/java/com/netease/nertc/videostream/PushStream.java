...
public class PushStream {
    ...
    private final Rect[] rectUsers = new Rect[2];
    private static int mWindowRight;
    private static int mWindowBottom;

    public PushStream(Context context, String taskId, long userId, NERtcVideoView mUserVv){
        this.context = context;
        this.taskId = taskId;
        this.userIds.add(userId);
        mWindowRight = mUserVv.getRight();
        mWindowBottom = mUserVv.getBottom() * 2;
        for(int i = 0;i < 2;i++)
            rectUsers[i] = new Rect(0,0, mUserVv.getWidth(), mUserVv.getHeight());
        rectUsers[1].offset(0, mUserVv.getHeight());

    };
    public void start(String url) {
        task = createLiveStreamTask(taskId, url); // 创建推流配置
        task.layout = createLiveStreamLayout(); // 创建视频布局
        task.layout.userTranscodingList = createUserTranscodingList(userIds, rectUsers); // 创建用户视频布局
    ...
    public void update(long userId, boolean add) {
        ...
        // 更新推流任务用户布局
        task.layout.userTranscodingList = createUserTranscodingList(userIds, rectUsers);
        // 更新推流任务
    ...
    private static NERtcLiveStreamLayout createLiveStreamLayout() {
        Rect rectLayout = new Rect(0, 0, mWindowRight, mWindowBottom);
        ...
        layout.backgroundImg = null; // 视频推流背景图
