<template>
  <div class="wrapper">
    <div class="content">
      ...
      <el-card class="control-container" body-style="backgroundColor: initial">
        <el-row>
          <el-col :span="8"><span class="demonstration">音频回调</span></el-col>
          <el-col :span="8"
            ><el-button type="primary" @click="enableAudioFrame"
              >开启</el-button
            ></el-col
          >
          <el-col :span="8"
            ><el-button type="primary" @click="disableAudioFrame"
              >关闭</el-button
            ></el-col
          >
        </el-row>
        <el-row>
          <el-col :span="8" :offset="8"
            ><el-button type="success" @click="downloadAudioData"
              >下载</el-button
            ></el-col
          >
          <el-col :span="8"
        ...
        <el-row>
          <el-col :span="8"><span class="demonstration">缓存时间</span></el-col>
          <el-col :span="16">
            <el-slider
              v-model="duration"
              :min="0.1"
  ...
  <script>
    ...
    import PCMPlayer from '../../lib/pcmplayer.js';
    export default {
        name: 'audioDump',
        data() {
            return {
        ...
        methods: {
            enableAudioFrame() {
                ...
                console.warn('localStream', this.localStream);
                this.localStream.enableAudioFrame(duration);
            },
            disableAudioFrame() {
                this.localStream.disableAudioFrame();
            },
            downloadAudioData() {
                if (this.audioDataBuffer && this.audioDataBuffer.data) {
                    const blob = new Blob([this.audioDataBuffer], { type: 'audio/pcm' });
            ...
            playAudioData() {
                const audioDataBuffer = this.audioDataBuffer;
                if (this.localStream && audioDataBuffer && audioDataBuffer.data) {
                    this.canplay = false;
                    let player = new PCMPlayer({
                        channel: 1,
                        sampleRate: audioDataBuffer.sampleRate,
                    });
                    player.feed(audioDataBuffer.data[0]);
                    player.play();
                    setTimeout(() => {
                        player.destroy().then(() => {
                            player = null;
            ...
            initLocalStream() {
                ...
                this.localStream.on('audio-data', (frame) => {
                    console.warn('audio data: ', frame);
                    this.audioDataBuffer = frame;
                });
