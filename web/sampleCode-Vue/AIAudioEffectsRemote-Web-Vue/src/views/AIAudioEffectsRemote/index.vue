  ...
  <div class="wrapper">
    <div class="content">
      ...
      <el-card class="control-container" body-style="backgroundColor: initial">
        ...
        <el-row>
          <el-col :span="8"
            ><span class="demonstration">远端流列表</span></el-col
          >
          <el-col :span="16">
            <el-select v-model="currentRemoteStream">
              <el-option
  ...
  methods: {
    async registerAIAudioEffects() {
      ...
      if (this.currentRemoteStream) {
        ...
        //注册plugin-load事件
        this.currentRemoteStream.on("plugin-load", (name) => {
        ...
        this.currentRemoteStream.on("audio-effect-enabled", () => {
          console.warn("AI音效已开启");
          this.currentRemoteStream.setAudioEffect(0, 2);
        });

        //插件注册失败时触发
        this.currentRemoteStream.on("plugin-load-error", (error) => {
          console.error("plugin-load-error", error);
        });
      } else {
        console.error("请先选择远端流");
      }
    },
    enableAudioEffect() {
      if (this.currentRemoteStream) {
        this.currentRemoteStream.enableAudioEffect();
        this.currentRemoteStream.audioEffectEnabled = true;
      } else {
        console.error("请先选择远端流");
      }
    },
    disableAudioEffect() {
      if (this.currentRemoteStream) {
        this.currentRemoteStream.disableAudioEffect();
        this.currentRemoteStream.audioEffectEnabled = false;
      } else {
        console.error("请先选择远端流");
      }
    },
    unregisterAIAudioEffects() {
      if (this.currentRemoteStream) {
        this.currentRemoteStream.unregisterPlugin("AIAudioEffects");
