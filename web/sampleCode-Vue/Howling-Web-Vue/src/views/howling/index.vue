    ...
    <div class="content">
      ...
      <el-card class="control-container" body-style="backgroundColor: initial">
        <el-row>
          ...
          <el-col :span="8">
            <el-button type="primary" @click="registerAIHowling"
              >注册</el-button
            >
          </el-col>
          <el-col :span="8">
            <el-button
              type="success"
              :disabled="!registerHowling"
              @click="unregisterAIHowling"
              >销毁</el-button
            >
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="8"><span class="demonstration">检测结果</span></el-col>
          <el-col :span="16">
            <el-table
              :data="howlingStatus"
...
export default {
  ...
  methods: {
    ...
    enableAudioEffect() {
      this.localStream.enableAIhowling();
      this.openHowling = true;
    },
    disableAudioEffect() {
      this.localStream.disableAIhowling();
      this.openHowling = false;
    },
    unregisterAIHowling() {
      this.localStream.unregisterPlugin("AIhowling");
      this.registerHowling = false;
      this.howlingStatus = [];
    },
    addHowlingStatus(info) {
      console.log("howling status: ", info);
      this.howlingStatus.unshift({
        time: new Date().toLocaleTimeString(),
        result: info ? "检测到啸叫" : "未检测到啸叫",
      });
    },
    getToken() {
      return getToken({
    ...
    initLocalStream() {
      ...
      this.localStream.on("plugin-load", (name) => {
        if (name === "AIhowling") {
          console.log("register AIHowling success");
          this.registerHowling = true;
        }
      });
      ...
      //注册啸叫回调
      this.localStream.onAudioHasHowling(this.addHowlingStatus);
